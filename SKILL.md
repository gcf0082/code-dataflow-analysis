...
分析开始前与过程中**主动**检索可用的既有知识，命中即引用，不重复 derive。

**来源**：同 skill 过往产出（兄弟/父目录里的 `methods-verified.md` / `flow-Fx.md` / `index.md`）；外部工具结构化输出（SAST、CodeQL、依赖清单等）；项目文档中关于具体方法、协议、不变式的陈述。

**发现**：默认从当前分析输出目录起递归向上扫到仓库根；用户可显式传入一个 knowledge 路径覆盖默认。

**引用**：在受益的步骤/校验/转换行末尾加 `[已知：见 <相对路径>]`，例如 `... IpUtils.isValidIpv4(s) ... [已知：见 ../auth-flow/methods-verified.md]`。

**冲突时重做**：默认信任既有记录；走到该方法/路径时本次仍读一眼实现——一致则保留引用，不一致则按当前观察重做、把更新写入本次的 `methods-verified.md`，旧引用标 `[已过时]`。

**外部工具与文档**：引用其原位置（文件/小节/行号）；本次报告不二次签收其结论，只告知读者"这条事实有外部支撑"。
