...
走入一个通用工具方法（`*Utils` / `*Helper` 类的校验、规范化、编解码、解析等，或独立辅助函数）并完整核对、行为与名称契约相符时，追加一条到 `utils-verified.md`（每核对完一个立即落盘）。下次分析撞到同样方法可直接引用，不必重做。
...
简单情形下 `utils-verified.md` 也单独成文件，不内联到 `report.md`——固定命名便于跨任务复用。

每分析完一条 flow / 一个独立小模块 / 一个工具方法核对就**立即落盘**，不要囤到最后。
