...
分析过程中如果走入一个**通用工具方法**（典型：`*Utils` / `*Helper` 类的校验、规范化、编解码、解析等方法，或独立辅助函数；名字暗示通用语义如 `isValid*` / `normalize*` / `escape*` / `encode*` / `decode*` / `parse*` 等），并且**完整核对了实现、结论是行为与方法名暗示的契约相符**，把这次核对单独追加到 `utils-verified.md` 一条。每核对完一个就立即落盘，不要囤到最后。这样下一次分析撞到同样方法可以直接引用，不必重做。

- **要记**：可被总结为"做 X"且实际就是做 X 的方法。
- **不记**：名字像 utility 但夹杂业务逻辑的；只看了一半未完整核对的；行为与名称不符的（后者属于事实陈述，放 Open Questions 或在 flow 步骤里说明，不进 verified 表）。
...
- **复杂**（多 flow / 长链路 / 多 Open Questions）：
  ...
  ├── flow-F2.md
  ├── utils-verified.md # 可选：通用工具方法核对记录（追加增量）
  └── ...
  ```

简单情形（单文件 `report.md`）下，如果有工具方法核对记录，仍把 `utils-verified.md` 单独成文件放在同目录——固定命名便于跨任务复用，不要内联进 `report.md`。

每分析完一条 flow（或可独立成立的小模块、或一个工具方法的核对）就**立即落盘**，不要囤到最后；后续补充追加文件、追写 `index.md` 即可。

每个 `flow-Fx.md` **自包含**：在文件内部重述涉及的 source / sink 摘要、调用链、步骤、校验、转换、最终表达式，不依赖 `index.md` 也能独立看懂。被多条 flow 共用的代码可在多个文件里各自出现，措辞可不同——目的是让 flow 之间互不干扰。
...
> 此文件追加增量；每条记录一个被核对过、实现与方法名暗示的契约相符的通用工具方法。
...
字段固定为：位置 / 预期行为 / 实际逻辑 / 边界 / 结论 五项。"结论"统一以"符合预期，…"开头，便于后续分析检索。
